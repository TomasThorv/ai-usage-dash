...
export function StepProgress({ step, total, labels }: StepProgressProps): ReactNode {
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-[11px] uppercase tracking-wide text-text-faint">
        {labels.map((label, i) => (
          <span key={label} className={i + 1 <= step ? "text-text-primary" : "text-text-faint"}>
            {i + 1}. {label}
          </span>
        ))}
      </div>
